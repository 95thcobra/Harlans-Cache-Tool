...
public class RSInterface {
	...
	private final Object[] method623(byte b, ByteBuffer buffer) throws IOException {
		...
		for (int i_4_ = 0; i_4_ < i; i_4_++) {
			...
			if (i_5_ == 0) {
				...
				objects[i_4_] = ByteBufferUtils.getJagexStringToString(buffer);
			}
		}
		if (b != -82) {
			return null;
	...
	final void loadData(ByteBuffer buffer) throws IOException, BufferUnderflowException {
		...
		if ((0x80 & contentType) != 0) {
			contentType &= 0x7f;
			aString859 = ByteBufferUtils.getJagexStringToString(buffer);
		}
		...
		if (contentType == 4) { //text
			...
			aString781 = ByteBufferUtils.getJagexStringToString(buffer);
			anInt845 = buffer.get();
			anInt722 = buffer.get();
			anInt857 = buffer.get();
		...
		aString888 = ByteBufferUtils.getJagexStringToString(buffer);
		...
		int i_44_ = i_42_ >> 4;
				if (i_43_ > 0) {
					...
					for (int i_45_ = 0; i_45_ < i_43_; i_45_++)
						aStringArray842[i_45_] = ByteBufferUtils.getJagexStringToString(buffer);
				}
				if (i_44_ > 0) {
				...
				aString830 = ByteBufferUtils.getJagexStringToString(buffer);
				...
				anInt770 = buffer.get();
				anInt882 = buffer.get();
				aString879 = ByteBufferUtils.getJagexStringToString(buffer);
				int i_49_ = -1;
				...
				if (-1 >= (i_34_ ^ 0xffffffff)) {
					...
					for (int i_55_ = 0; i_54_ > i_55_; i_55_++) {
						...
						buffer.get();
						String string = ByteBufferUtils.getJagexStringToString(buffer);
